...
var links_httpLink_dist_trpcClientLinksHttpLink = require('../../../dist/createTRPCClient-181ea46c.cjs.prod.js');
  ...
  return function (runtime) {
    ...
    var fetcher = function fetcher(type) {
      return function (keyInputPairs) {
        ...
        var _httpRequest = links_httpLink_dist_trpcClientLinksHttpLink.httpRequest({
          url: url,
          input: input,
          path: path,
          runtime: runtime,
    ...
    return function (_ref3) {
      ...
      onDestroy(function () {
        if (!done) {
          prev(links_httpLink_dist_trpcClientLinksHttpLink.TRPCClientError.from(new DOMException('The operation was aborted.', 'AbortError')));
        }
      ...
      promise.then(function (result) {
        if (!done) {
          prev(result.ok ? result : links_httpLink_dist_trpcClientLinksHttpLink.TRPCClientError.from(result));
        }
      }).catch(function (err) {
        if (!done) {
          prev(links_httpLink_dist_trpcClientLinksHttpLink.TRPCClientError.from(err));
        }
      }).finally(function () {
        done = true;
      });

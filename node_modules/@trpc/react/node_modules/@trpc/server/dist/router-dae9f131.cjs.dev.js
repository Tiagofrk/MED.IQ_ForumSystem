...
var index = require('./index-4fbf1837.cjs.dev.js');
var once = require('./once-34ddee5a.cjs.dev.js');
...
class Procedure {
  ...
  parseInput(rawInput) {
    try {
      ...
      const message = once.getMessageFromUnkownError(originalError, 'Input Validation Error');
      throw once.inputValidationError(message, {
        originalError
      });
    }
  }
...
function createProcedure(opts) {
  ...
  return new ProcedureWithoutInput({
    ...
    inputParser(input) {
      if (input != null) {
        throw once.inputValidationError('No input expected');
      }
...
class Router {
  ...
  async invoke({
    ...
    if (!procedure) {
      throw once.notFoundError(`No such ${type} procedure "${path}"`);
    }
